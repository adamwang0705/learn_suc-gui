...
def index(request):
    ...
    if not request.session.get('displaying_type_name', None):
        request.session['displaying_type_name'] = default_displaying_type_name
    if not request.session.get('displaying_fl', None):
        request.session['displaying_fl'] = default_displaying_fl
    if not request.session.get('displaying_page', None):
        request.session['displaying_page'] = 1

    if request.method == 'GET':
        if request.GET.get('filter_type_select'):
            request.session['displaying_type_name'] = request.GET.get('filter_type_select')
            request.session['displaying_page'] = 1
        if request.GET.get('filter_fl_select'):
            request.session['displaying_fl'] = request.GET.get('filter_fl_select')
            request.session['displaying_page'] = 1
        if request.GET.get('page'):
            request.session['displaying_page'] = request.GET.get('page')
        ...
        if request.POST.getlist('delete_items_multiple_select'):  # Handle delete_items_form
        ...
        if request.POST.get('searched_item_id'):  # Handle search_item_form
            # Receive add item
    ...
    displaying_items_complete = Item.objects.filter(type=type_name2type[request.session.get('displaying_type_name')],
                                                    name_fl=request.session['displaying_fl']).order_by('name')
    displaying_items_complete_info = []
    for item in displaying_items_complete:
        displaying_items_complete_info.append({'item_id': item.id,
                                               'name': item.name,
                                               'type_name': type_names_[item.type]})

    displaying_items_info_paginator = Paginator(displaying_items_complete_info, 500)
    try:
        displaying_items_info = displaying_items_info_paginator.page(request.session['displaying_page'])
    except PageNotAnInteger:
        displaying_items_info = displaying_items_info_paginator.page(1)
    except EmptyPage:
        displaying_items_info = displaying_items_info_paginator.page(displaying_items_info_paginator.num_pages)
